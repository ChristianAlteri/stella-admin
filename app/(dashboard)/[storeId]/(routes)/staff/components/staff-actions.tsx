...
export default function StaffActions({ data }: { data: Staff }) {
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/staff/${data.id}`);
  ...
  const handleCardClick = () => {
    router.push(`/${params.storeId}/staff/${data.id}/details`);
  };
    ...
    <>
      <div className="flex justify-end space-x-2 mb-4 text-muted-foreground">
        <div className="flex flex-data gap-2">
          ...
          <Link
            href={`/${params.storeId}/products?staffId=${data.id}`}
            passHref
          >
        ...
        <Button
          variant="outline"
          size="icon"
          onClick={() => router.push(`/${params.storeId}/staff/${data.id}`)}
          aria-label="Edit"
        >
