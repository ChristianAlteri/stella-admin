...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/sub-categories/${data.id}`);
      toastSuccess('Subcategory deleted.');
      router.refresh();
    } catch (error) {
        toastError('Make sure you removed all products using this Subcategory first.');
    } finally {
      setOpen(false);
      setLoading(false);
    }
  ...
  const onCopy = (id: string) => {
    ...
    toastSuccess('Subcategory ID copied to clipboard.');
  }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`/${params.storeId}/sub-categories/${data.id}`)}
          >
